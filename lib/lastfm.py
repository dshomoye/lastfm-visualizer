...
class LastFMHelper:
    ...
    def __init__(
        self, api_key: Optional[str] = key, username: str = "sonofatailor"
    ) -> None:
        ...
        if not self.API_KEY and not app.config["TESTING"]:
            raise ValueError("No API KEY passed to LastFM or set in env")
    ...
    def get_scrobbles_in_period(
        ...
        return [
            ...
            if start_period <= scrobble.date <= end_period
        ]

    def _get_scrobbles_from_lf(self, payload: dict = {}) -> dict:
        page, total_pages = 0, 1
        if "scrobbles" not in self.SCROBBLES_CACHE:
            self.SCROBBLES_CACHE["scrobbles"] = []
        print(
            f"downloading scrobbles for {self.username}... started at {datetime.now()}"
        )
        while total_pages > page:
            page += 1
            try:
                scrobbles_in_page = self.__get_scrobbles_page(
                    page=page, payload=payload
                )
            except ScrobbleFetchFailed:
                self.db.set_user_update_to_min()
                raise ScrobbleFetchFailed
            parsed_scrobbles = self.__parse_scrobbles(
                scrobbles_in_page["recenttracks"]["track"]
            )
    ...
    def __parse_scrobbles(self, scrobbles: List[dict]) -> List[Scrobble]:
        ...
        for scrobble in scrobbles:
            ...
            if "date" in scrobble:
                s = Scrobble(
                    track=t, timestamp=int(scrobble["date"]["uts"])
                )  # type:ignore
            # set currently playing to now
                ...
                try:
                    if scrobble["@attr"]["nowplaying"] == "true":
                        s = Scrobble(
                            track=t, timestamp=int(datetime.now().timestamp())
                        )  # type:ignore
                except:
                    continue
    ...
    def __get_scrobbles_page(self, page: int, payload: dict = {}) -> dict:
        ...
        if "error" in r.json() and r.json()["error"] == 6:
            ...
            raise ScrobbleFetchFailed(
                f"An error occured getting srobbles from LastFM, response:{r.text}"
            )
        return r.json()
    ...
    def __do_request(self, http_method, payload):
        ...
        request_methods = {"GET": requests.get, "POST": requests.post}
        payload["format"] = "json"
