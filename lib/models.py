...
class Track(db.Model):
    ...
    def to_dict(self):
        return {"title": self.title, "album": self.album, "artist": self.artist}


class User(db.Model):
    __tablename__ = "users"
    __table_args__ = (UniqueConstraint("name"), {"mysql_row_format": "DYNAMIC"})
    id = Column(Integer, primary_key=True)
    name = Column(String(256))
    last_update = Column(DateTime)
    user_state = Column(String(128))
...
class Scrobble(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint("date", "time", "user_id"),
        {"mysql_row_format": "DYNAMIC"},
    )
    id = Column(Integer, primary_key=True)
    ...
    def to_dict(self):
        return {"date": self.datetime, "track": self.track.to_dict()}
