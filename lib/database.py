...
class DbHelper:
    ...
    def get_last_update(self):
        return (
            self.session.query(User).filter_by(name=self.user.name).first().last_update
        )

    def get_user_state(self):
        return (
            self.session.query(User).filter_by(name=self.user.name).first().user_state
        )

    def get_all_scrobbles_from_db(self) -> List[Scrobble]:
        return self.session.query(Scrobble).all()

    def get_scrobbles_in_period(
        self, start_period: datetime, end_period: datetime
    ) -> List[Scrobble]:
        result = (
            self.session.query(Scrobble)
    ...
    def get_top_tracks_for_period(
        self, start_period: datetime, end_period: datetime, limit: int = 5
    ) -> List[Dict[str, Any]]:
        result = (
            self.session.query(Track, func.count(Scrobble.timestamp).label("qty"))
            .join(Scrobble.track)
            .filter(Scrobble.datetime >= start_period)
            .filter(Scrobble.datetime <= end_period)
            .filter(Scrobble.user == self.user)
    ...
    def get_top_albums_for_period(
        self, start_period: datetime, end_period: datetime, limit: int = 5
    ) -> List[Dict[str, Any]]:
        result = (
            self.session.query(
                Track.album, Track.artist, func.count(Scrobble.timestamp).label("qty")
            )
            .join(Scrobble.track)
            .filter(Scrobble.datetime >= start_period)
            .filter(Scrobble.datetime <= end_period)
        ...
        for r in result:
            results.append({"played": r[2], "album": r[0], "artist": r[1]})
        return results
    ...
    def get_top_artists_for_period(
        ...
        result = (
            self.session.query(
                Track.artist, func.count(Scrobble.timestamp).label("qty")
            )
            .join(Scrobble.track)
            .filter(Scrobble.datetime >= start_period)
            .filter(Scrobble.datetime <= end_period)
        ...
        for r in result:
            results.append({"played": r[1], "artist": r[0]})
        return results

    def get_track_count_in_period(
        self, start_period: datetime, end_period: datetime, unit="days"
    ) -> Dict[str, int]:
        unit_map = {
            "hours": [Scrobble.date, func.extract("hour", Scrobble.time)],
            "days": [func.extract("day", Scrobble.date), Scrobble.date],
            "weekdays": [
                func.extract("year", Scrobble.date),
                func.extract("month", Scrobble.date),
                func.extract("dow", Scrobble.date).label("weekday"),
            ],
            "months": [
                func.extract("year", Scrobble.date),
                func.extract("month", Scrobble.date),
        ...
        r = (
            self.session.query(*unit_map[unit], count)
        ...
        transform_map: Dict[str, DateTime] = {
            ...
            "weekdays": lambda y, m, w: datetime(y, m, 1)
            + relativedelta(day=1, weekday=w),
    ...
    def add_track_to_db(self, track, title, album, artist) -> Track:
        t = (
            ...
            .filter_by(title=title, album=album, artist=artist)
            .first()
        )
        if t:
            return t
    ...
    def write_scrobbles_to_db(self, scrobbles: List[Scrobble]) -> None:
        ...
        for scrobble in scrobbles:
            ...
            t = self._add_or_ignore_to_db(
                Track, track, title=track.title, artist=track.artist, album=track.album
            )
            # t = self.add_track_to_db(track,title=track.title,artist=track.artist,album=track.album)
            scrobble.track = None
            self._add_or_ignore_to_db(
                Scrobble, scrobble, date=scrobble.date, time=scrobble.time
            )
            scrobble.track = t
